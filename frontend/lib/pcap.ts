...
function dedupeSipMessages(msgs: ParsedPacket[]): ParsedPacket[] {
  ...
  for (const m of sorted) {
    // Chave SEM porta nem direcao: copias do mesmo pacote (fio + duplicado HEP,
    // ou multi-interface) podem vir com porta diferente e ate src/dst trocados
    // pelo HEP, virando seta paralela/oposta pra mesma mensagem. Identifica pela
    // mensagem (metodo/status + CSeq + texto) + par de IPs ordenado.
    const ipPair = [m.srcIp ?? "", m.dstIp ?? ""].sort().join("><");
    const key = [ipPair, m.sipMethodOrStatus, m.sipCseqMethod, m.sipText].join("|");
 ...
 * ligação atendeu.
 *
 * IMPORTANTE: RTP NÃO é considerado "atendida". Early media (183 Session
 * Progress + RTP de ringback/IVR ANTES do atendimento) faz áudio fluir sem
 * 200 OK — só o 200 (em qualquer perna) significa atendida. Um bump por RTP
 * marcava como IN CALL chamadas que estavam só TOCANDO com early media, e
 * ainda divergia da lista (que não calcula RTP). Pra "houve mídia?" a UI usa
 * `hasEarlyMedia` (rtpFlows) à parte, sem mexer no estado de sinalização.
 */
...
export function groupState(group: CallGroup): SipDialogState {
